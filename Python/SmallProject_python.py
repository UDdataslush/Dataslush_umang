...
def filter_weather_data(data, filter_option):
    """Filter weather data based on the given option."""
    current_date = datetime.now()

    if filter_option == 'today':
        return data if data['date'] == current_date.strftime('%Y-%m-%d') else None

    elif filter_option == 'week':
        one_week_ago = current_date - timedelta(days=7)
        return data if datetime.strptime(data['date'], '%Y-%m-%d') >= one_week_ago else None

    elif filter_option == 'month':
        one_month_ago = current_date - timedelta(days=30)
        return data if datetime.strptime(data['date'], '%Y-%m-%d') >= one_month_ago else None

    return data

def main_cli(city, output_file, output_format, filter_option):
    ...
    try:
        ...
        # Apply the filter 
        if filter_option:
            weather_data = filter_weather_data(weather_data, filter_option)
            if not weather_data:
                print(f"No weather data matches the filter: {filter_option}")
                return
...
def main():
    ...
    parser.add_argument("--filter", choices=['today', 'week', 'month'], help="Filter data by date (today, week, month)")

    args = parser.parse_args()

    if args.city and args.output_file and args.output_format:
        # Use argparse arguments if provided
        main_cli(args.city, args.output_file, args.output_format, args.filter)
    else:
        ...
        filter_option = input("Enter the filter (today, week, month) or leave blank for no filter: ").strip().lower()
        if filter_option not in ['today', 'week', 'month', '']:
            print("Invalid filter option.")
            return

        main_cli(city, output_file, output_format, filter_option if filter_option else None)
